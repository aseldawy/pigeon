...
public class TestSJPlaneSweep extends TestCase {
  ...
  public void testShouldAutomaticallyDetectGeometryColumns() throws Exception {
    // Create polygons
    ArrayList<String[]> data = new ArrayList<String[]>();
    data.add(new String[] {"1", "1", "LINESTRING(0 0, 2 2)"});
    data.add(new String[] {"1", "2", "LINESTRING(2 1, 0 4)"});
    ...
    String query = "A = LOAD 'file:" + datafile + "' as (gid: int, id: int, geom);\n" +
      ...
      "rdataset = FILTER A BY gid == 2;\n" +
      "datasets = COGROUP ldataset ALL, rdataset ALL;\n"+
      "C = FOREACH datasets GENERATE FLATTEN("+SJPlaneSweep.class.getName()+"(ldataset, rdataset, null));\n" +
      "D = FOREACH C GENERATE $1, $4;\n";
    pig.registerQuery(query);
    Iterator<?> it = pig.openIterator("D");
  ...
  public void testShouldWorkWithDuplicateAvoidance() throws Exception {
    // Create polygons
    ArrayList<String[]> data = new ArrayList<String[]>();
    data.add(new String[] {"1", "1", "LINESTRING(0 0, 2 2)"});
    data.add(new String[] {"1", "2", "LINESTRING(2 1, 0 4)"});
    ...
    String datafile = TestHelper.createTempFile(data, "\t");
    datafile = datafile.replace("\\", "\\\\");
    String mbr = MakeBox.class.getName()+"(2.5, 2.5, 5.5, 5.5)";
    PigServer pig = new PigServer(LOCAL);
    String query = "A = LOAD 'file:" + datafile + "' as (gid: int, id: int, geom);\n" +
        "ldataset = FILTER A BY gid == 1;\n" +
        "rdataset = FILTER A BY gid == 2;\n" +
        "datasets = COGROUP ldataset ALL, rdataset ALL;\n"+
        "C = FOREACH datasets GENERATE FLATTEN("+SJPlaneSweep.class.getName()+"(ldataset, rdataset, "+mbr+"));\n" +
        "D = FOREACH C GENERATE $1, $4;\n";
    pig.registerQuery(query);
    Iterator<?> it = pig.openIterator("D");
  ...
  public void testShouldWorkWithColumnNames() throws Exception {
    // Create polygons
    ArrayList<String[]> data = new ArrayList<String[]>();
    data.add(new String[] {"1", "1", "LINESTRING(0 0, 2 2)"});
    data.add(new String[] {"1", "2", "LINESTRING(2 1, 0 4)"});
    ...
    String query = "A = LOAD 'file:" + datafile + "' as (gid: int, id: int, geom);\n" +
      ...
      "rdataset = FILTER A BY gid == 2;\n" +
      "datasets = COGROUP ldataset ALL, rdataset ALL;\n"+
      "C = FOREACH datasets GENERATE FLATTEN("+SJPlaneSweep.class.getName()+"(ldataset, rdataset, null, ldataset.geom, rdataset.geom));\n" +
      "D = FOREACH C GENERATE $1, $4;\n";
    pig.registerQuery(query);
    Iterator<?> it = pig.openIterator("D");
  ...
  public void testShouldWorkWithMacro() throws Exception {
    // Create records
    ArrayList<String[]> data = new ArrayList<String[]>();
    data.add(new String[] {"1", "1", "LINESTRING(0 0, 2 2)"});
    data.add(new String[] {"1", "2", "LINESTRING(2 1, 0 4)"});
    ...
    String query = String.format("IMPORT '%s';\n", import_file.getPath()) +
      ...
      "ldataset = FILTER A BY gid == 1;\n" +
      "rdataset = FILTER A BY gid == 2;\n" +
      "C = PBSM(ldataset, rdataset, 'geom', 'geom');\n"+
      "D = FOREACH C GENERATE $1, $4;\n";
    pig.registerQuery(query);
    Iterator<?> it = pig.openIterator("D");
